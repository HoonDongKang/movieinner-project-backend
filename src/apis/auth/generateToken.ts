...
const generateToken = async (
  ...
  const refreshTokenExpiredDate = new Date(Date.now() + 3600 * 1000 * 24 * 180); // 6개월
  try {
    const { email } = params;
    ...
    if (getResponse[0]) {
      await connection.run(
        ...
        [accessToken, expiredDate, refreshToken, refreshTokenExpiredDate, email]
      );
    } else {
      await connection.run(
        `INSERT INTO user_token(email,access_token,expires_in,refresh_token,refresh_token_expires_in) VALUES(?,?,?,?,?)`,
        [email, accessToken, expiredDate, refreshToken, refreshTokenExpiredDate]
      );
    }
  } catch (e: any) {
