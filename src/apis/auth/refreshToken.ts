    ...
    try {
        ...
        if (getResponse[0]) {
            ...
            if (refreshTokenPayload.accessToken === accessToken) {
                ...
                const refresh_token_expires_in_number = Date.parse(
                    refresh_token_expires_in
                )
                // refresh token의 만료가 1개월 미만일 경우
                if (
                    refresh_token_expires_in_number >
                    now + 3600 * 1000 * 24 * 30
                ) {
                    ...
                    newRefreshTokenExpiry = refreshTokenExpiredDate
                } else {
                    //1개월 이상일 경우 기존 refresh token의 만료기간을 유지
                    newAccessTokenPayload = {
                        email,
                        idx,
                        nickname,
                    ...
                    newRefreshTokenExpiry = refresh_token_expires_in
                }
            } else {
                throw 'E0005'
            }
